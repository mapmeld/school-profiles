...
# munis = json.loads(open('../data/municipios.geojson', 'r').read())
# schools_by_muni = {}
# for geo in munis["features"]:
...
# exp = open('export_munis.geojson', 'w')
# exp.write(json.dumps(munis, separators=(',', ':')))

deptos = json.loads(open('../data/departamentos.geojson', 'r').read())
schools_by_depto = {}
for geo in deptos["features"]:
    #print(geo["properties"])
    dept = geo["properties"]["NAME_1"].upper()
    dept = dept.replace('Á', 'A').replace('Ñ','N').replace('Ó','O').replace('Í', 'I').replace('É', 'E')
    ...
    #     print("No schools? " + dept + " > " + muni)
    geo["properties"]["rates"] = student_status
exp = open('export_deptos.geojson', 'w')
exp.write(json.dumps(deptos, separators=(',', ':')))
